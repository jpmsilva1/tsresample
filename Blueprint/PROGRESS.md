...
| 13 | Release | 12 | 0, 11, 12 | Blocked: remote actions reserved for the maintainer — step 0 (PyPI Trusted Publishing for `tsresample`, `pypi` environment, branch protection), pushing, and the `v0.1.0` tag; and G4 R1 (DS19) is red, which blocks a release under QUALITY_GATES §3 until the maintainer decides (open item) | local branch `node/13-release`: `.github/workflows/release.yml` (G4 must run, never skip; sdist + pure wheel; clean-venv install with no pandas; tag/version check; OIDC publish). Wheel verified locally in a fresh venv |
